...
import (
	...
	"github.com/pdffillerdocker/sso-gsuite-user-sync/internal/aws"
	"github.com/pdffillerdocker/sso-gsuite-user-sync/internal/config"
	"github.com/pdffillerdocker/sso-gsuite-user-sync/internal/google"
...
func DoSync(ctx context.Context, cfg *config.Config) error {
    ...
    creds, err := aws.S3getobject(cfg.SecretBucket, cfg.GoogleCredentials, cfg.SecretBucketRegion)
	if err != nil {
		return err
	}

    token, err := aws.S3getobject(cfg.SecretBucket, cfg.SCIMAccessToken, cfg.SecretBucketRegion)
	if err != nil {
		return err
	}
